...
public:
    ...
    bool update(int param)
    {
        if(millis() - lastStepTime >= STEP_DELAY)
        {
            ...
            for (int k=0; k<NUM_LEDS / 2; k++) {                              // For each of the LED's in the strand, set a brightness based on a wave as follows:

                int colorIndex = cubicwave8((k*23)+thisPhase)/2 + cos8((k*15)+thatPhase)/2;           // Create a wave and add a phase change and add another wave with its own phase change.. Hey, you can even change the frequencies if you wish.
                int thisBright = qsuba(colorIndex, beatsin8(7,0,96));                                 // qsub gives it a bit of 'black' dead space by setting sets a minimum value. If colorIndex < current value of beatsin8(), then bright = 0. Otherwise, bright = colorIndex..
                CRGB color = ColorFromPalette(OceanColors_p, colorIndex, thisBright, LINEARBLEND);  // Let's now add the foreground colour.
                ledControl->leds[k] = color;
                ledControl->leds[NUM_LEDS - k] = color;
            }
        }
